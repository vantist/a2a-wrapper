/**
 * Config schema validation tests — a2a-copilot
 *
...
function stripComments<T>(value: T): T {
  // The example configs use top-level `$comment` and `$schema` keys for
  // editor metadata. Schemas generated by ts-json-schema-generator with
  // `additionalProperties: false` reject extra keys, so strip them
  // before validating. This mirrors what a config loader would do.
...
describe("AgentConfig JSON schema", () => {
  ...
  it("accepts the filesystem-assistant agent config", () => {
    const cfg = stripComments(loadJson("agents/filesystem-assistant/config.json"));
    const ok = validate(cfg);
    expect(validate.errors, JSON.stringify(validate.errors, null, 2)).toBeNull();
    expect(ok).toBe(true);
  });
  ...
  it("rejects a sub-agent name with disallowed characters", () => {
    const cfg = {
    ...
    // The pattern lives on `SubAgentConfig.name`. Even if the schema
    // generator didn't emit the pattern, validation should at least
    // accept the shape. We only assert pattern enforcement when the
    // generated schema actually includes it.
    const ok = validate(cfg);
    if (!ok) {
      // Good — the schema enforced the pattern.
      expect(validate.errors).toBeDefined();
    }
  });
